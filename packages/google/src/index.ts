...
import { Calendar, CalendarInputError } from "./calendar.js";
...
function failure(error: unknown): ToolExecutionResult {
  ...
  if (error instanceof CalendarInputError) return { ok: false, effectStatus: "not_applicable", error: { code: "invalid_input", message: error.message, retryable: false } };
...
export function createPlugin(context: PluginSetupContext, options: CreatePluginOptions = {}): PluginInstance {
  ...
  const eventProperties = { summary: { type: "string", description: "Event title" }, start: { type: "string", description: "Start date-time in ISO 8601 format, or YYYY-MM-DD when all_day is true" }, end: { type: "string", description: "End date-time in ISO 8601 format, or the exclusive end date (usually the next day) when all_day is true" }, all_day: { type: "boolean", description: "Use date-only all-day boundaries. For updates, provide this with both start and end when setting or changing the event's all-day mode." }, description: { type: "string", description: "Event description" }, location: { type: "string", description: "Event location" }, calendar_id: { type: "string", description: "Calendar ID (default: primary)" } };
  const eventFields = (input: JsonObject) => ({ ...(str(input.summary) !== undefined ? { summary: String(input.summary) } : {}), ...(str(input.start) ? { start: String(input.start) } : {}), ...(str(input.end) ? { end: String(input.end) } : {}), ...(typeof input.all_day === "boolean" ? { allDay: input.all_day } : {}), ...(typeof input.description === "string" ? { description: input.description } : {}), ...(typeof input.location === "string" ? { location: input.location } : {}) });
  ...
  const tools: ToolDefinition[] = [
    ...
    tool({ name: "google_calendar_create_event", description: "Create a timed or all-day calendar event. For an all-day event set all_day=true and use YYYY-MM-DD dates; end is exclusive, so a one-day event ends on the next date.", inputSchema: { type: "object", additionalProperties: false, required: ["summary", "start", "end"], properties: eventProperties }, policy: write("google.calendar.write", "non_idempotent", input => ({ kind: "google-calendar", id: calendarId(input) })), execute: (input, { signal }) => calendar.createEvent(calendarId(input), eventFields(input), signal) }),
    tool({ name: "google_calendar_update_event", description: "Update an existing calendar event. Only provided fields will be changed. To set or convert an all-day event, provide all_day plus both date boundaries; an all-day end date is exclusive.", inputSchema: { type: "object", additionalProperties: false, required: ["event_id"], properties: { event_id: { type: "string", minLength: 1, description: "Event ID to update" }, ...eventProperties } }, policy: write("google.calendar.write", "idempotent", input => ({ kind: "google-calendar", id: calendarId(input) })), execute: (input, { signal }) => calendar.updateEvent(calendarId(input), String(input.event_id), eventFields(input), signal) }),
