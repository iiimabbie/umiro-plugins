...
Use unknown and a low confidence when uncertain. Set userExplicitlyRequestedExecution only when the user's original text explicitly asks for execution; it does not grant authorization. Set shouldReply only according to whether a response is useful; it does not override host hard-deny policy. Select only tools from the supplied candidate list; include tools that could perform, prepare for, or verify the requested work, including prerequisite inspection tools for an explicit action. Missing target details do not make an otherwise applicable tool irrelevant. Do not output permissions, arguments, or a suggested reply.`;
